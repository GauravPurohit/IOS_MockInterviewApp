...
<?php
    ...
    class MySqlDaoServer {
        ...
        {
            $returnValue = array();
            $sql = "select * from 1840235_mockintr.users where username='" . $username . "'";
        ...
        {
            $returnValue = array();
            $salt = "select salt from 1840235_mockintr.users where username='" . $username . "'";
        ...
        public function getUserDetailsWithPassword($username, $userPassword, $passDetails)
        {
            ...
            $sql = "select * from 1840235_mockintr.users where username='" . $username . "' and password='" . $userPasswordNew . "'";
            $result = $this->conn->query($sql);
            if ($result != null && (mysqli_num_rows($result) >= 1)) {
                $row = $result->fetch_array(MYSQLI_ASSOC);
                if (!empty($row)) {
        ...
        public function registerUser($username, $password, $salt)
        {
            ...
            $sql = "insert into 1840235_mockintr.users set username=?, password=?, salt=?";
